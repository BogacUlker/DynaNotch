...
class MusicManager: ObservableObject {
    ...
    // Lyrics are now managed by LyricsManager.shared
    // These computed properties preserve backward compatibility for any remaining references.
    var currentLyrics: String { LyricsManager.shared.plainText }
    var isFetchingLyrics: Bool { LyricsManager.shared.isFetching }
    var syncedLyrics: [LyricLine] { LyricsManager.shared.syncedLines }
    ...
    private func updateFromPlaybackState(_ state: PlaybackState) {
        ...
        if hasContentChange {
            ...
            // Delegate lyrics fetching to LyricsManager
            LyricsManager.shared.onTrackChanged(
                title: state.title,
                artist: state.artist,
                album: state.album,
    ...
    // MARK: - Lyrics (delegated to LyricsManager)

    /// Backward-compatible lyric line lookup. Uses LyricsManager internally.
    func lyricLine(at elapsed: Double) -> String {
        return LyricsParser.lineText(in: LyricsManager.shared.syncedLines, at: elapsed)
    }
